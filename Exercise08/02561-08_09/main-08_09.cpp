...
mat4 getLightProjection() {
	float l = length(teapotPosition - lightPosition);

	float angle = asin((teapotBoundingRadius / l));

	return Perspective(angle*2*RADIAN_TO_DEGREE, 1., 1., 400.); 
}

mat4 getLightView() {
	return LookAt(lightPosition,teapotPosition,vec3(1,0,0));
}
