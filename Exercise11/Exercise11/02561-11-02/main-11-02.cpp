...
void setupCurve(){
	...
	float cornerWeight = sqrt(2.0)/2.0;
	float middleWeight = 1.0;
	vec4 controlPoints [numberOfControlPoints] = {
		vec4( -1, 0,0,  middleWeight),
		vec4( -1, 1,0,  cornerWeight),
		vec4( 0, 1,0,   middleWeight),
		vec4( 1, 1,0,	cornerWeight),
	...
	//vec3 controlPoints [numberOfControlPoints] = {
	//	vec3( -1, 0,0),
	//	vec3( 1, 0.5,0),
	//	vec3( 0, 1,0),
	//	vec3(-1, 1,0)};
	...
	float knotVector[numberOfKnots] = {0, 0, 0, M_PI/2.0, M_PI/2.0, M_PI, M_PI, (3.0*M_PI)/2.0, (3.0*M_PI)/2.0, 2.0*M_PI, 2.0*M_PI, 2.0*M_PI};//{0,0,0,0,1,1,1,1};
